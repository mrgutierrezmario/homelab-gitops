...
| `NodeNotReady` in pod events | the Lima VM paused — usually the Mac slept. Everything recovers on its own, but while it is out, nothing syncs. Production runs on the same Mac, so it should not be sleeping: check `pmset -g` |
